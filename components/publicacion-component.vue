...
const { data, pending, error, refresh } = await useFetch(`/api/publicaciones?id=${id}`, {
    // pick: ['title']
})
...
<template>
    ...
    <div>
        <h2 class="text-3xl font-medium uppercase mb-2">{{ data.publicacion.producto }}</h2>
        ...
        <div class="flex items-baseline mb-1 space-x-2 font-roboto mt-4">
            <p class="text-xl text-primary font-semibold">${{ data.publicacion.precio }}</p>
        ...
        <div class="space-y-2 mt-4">
            ...
            <p class="text-gray-600">{{ data.publicacion.descripcion }}.</p>
        </div>
        <div class="mt-4">
        ...
        <div class="mt-6 flex gap-3 border-b border-gray-200 pb-5 pt-5">
                <button @click="agregarAlCarrito(data.publicacion)"
